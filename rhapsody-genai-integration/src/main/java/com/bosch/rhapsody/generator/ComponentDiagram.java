        ...
        if (components != null) {
            for (int i = 0; i < components.length(); i++) {
                try {
                    ...
                    String componentName = componentObject.optString(Constants.JSON_ALIAS_NAME, "")
                            .replaceAll("[^a-zA-Z0-9]", "_");
                    String componentLabel = componentObject.getString(Constants.JSON_NAME).replaceAll("[^a-zA-Z0-9]",
                            "_");
        ...
        if (actors != null) {
            for (int i = 0; i < actors.length(); i++) {
                try {
                    ...
                    IRPActor rhapsodyActor = ComponentDiagramUtil.addActor((IRPPackage) container, actorName,
                            basePackage);
        ...
        if (interfaces != null) {
            for (int i = 0; i < interfaces.length(); i++) {
                try {
                    ...
                    String interfaceName = interfaceObject.optString(Constants.JSON_ALIAS_NAME, "").replaceAll(
                            "[^a-zA-Z0-9]",
                            "_");
                    String interfaceLabel = interfaceObject.getString(Constants.JSON_NAME).replaceAll("[^a-zA-Z0-9]",
                            "_");
            ...
            for (int i = 0; i < calls.length(); i++) {
                try {
                    ...
                    if (fromElem != null && toElem != null) {
                        if (fromElem instanceof IRPClass
                            ...
                            IRPFlow flow = basePackage.addFlows("flow_" + fileCount + "_" + i);
                            flow.setEnd1(fromElem);
                            flow.setEnd2(toElem);
                            flow.setDisplayName(description);
                        }
                    } else {
                        if (fromElem == null) {
                            Constants.rhapsodyApp.writeToOutputWindow(Constants.LOG_TITLE_GEN_AI_PLUGIN,
                                    "ERROR: Source element not found: " + from + Constants.NEW_LINE);
                        }
                        if (toElem != null) {
                            Constants.rhapsodyApp.writeToOutputWindow(Constants.LOG_TITLE_GEN_AI_PLUGIN,
                                    "ERROR: Target element not found: " + to + Constants.NEW_LINE);
                        }
                    }
                } catch (Exception e) {
                    Constants.rhapsodyApp.writeToOutputWindow(Constants.LOG_TITLE_GEN_AI_PLUGIN,
